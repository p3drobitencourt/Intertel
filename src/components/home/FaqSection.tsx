...
export default function FaqSection({ onContactSupport, category ='geral' }: FaqSectionProps) {
 ...
 return (
 <section className="py-6 md:py-5 bg-zinc-50 border-t border-slate-200 relative z-10" id="faq">
 <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
 <div className="text-center space-y-3 max-w-2xl mx-auto mb-12">
 <span className="block text-xs font-mono text-blue-600 uppercase tracking-[0.25em] font-bold mb-2">DÚVIDAS FREQUENTES</span>
 ...
 <div
 key={idx}
 className="bg-white rounded-2xl border border-slate-200 hover:border-slate-300 transition-all overflow-hidden shadow-sm hover:shadow-xl hover:-translate-y-1"
 >
 <button
 ...
 {isOpen ? (
 <ChevronUp className="w-7 h-7 text-blue-500 shrink-0" aria-hidden="true" />
 ) : (
 <ChevronDown className="w-7 h-7 text-slate-600 shrink-0" aria-hidden="true" />
 )}
 </button>
 ...
 role="region"
 className="px-6 pb-6 text-slate-600 text-xs sm:text-sm leading-relaxed border-t border-slate-200 pt-4 animate-in fade-in slide-in-from-top-1">
 {item.a}
 </div>
 )}
 </div>
 ...
 {/* Quick FAQ CTA */}
 <div className="mt-6 text-center p-6 bg-slate-100/20 rounded-2xl border border-slate-300 flex flex-col sm:flex-row items-center justify-between gap-4">
 <div className="text-left space-y-1">
 <h4 className="font-bold text-slate-900 text-sm">Não encontrou a resposta que procurava?</h4>
 <p className="text-slate-600 text-xs">Fale diretamente com nossa ouvidoria e equipe de suporte pelo WhatsApp.</p>
 </div>
 <button
